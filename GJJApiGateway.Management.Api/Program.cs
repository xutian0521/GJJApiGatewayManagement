...
Console.WriteLine($"当前主机 IP: {hostIP}");
...
builder.Services.AddOpenTelemetry()
    ...
    .WithTracing(tracing => tracing
        ...
        {
            // 推送到 Jaeger
            options.Endpoint = new Uri("http://localhost:4317"); // Jaeger 的 OTLP gRPC 端点
            options.Protocol = OpenTelemetry.Exporter.OtlpExportProtocol.Grpc; // 使用 gRPC 协议
        })
        .AddOtlpExporter(options =>
        {
            // 推送到 Seq
            options.Endpoint = new Uri("http://localhost:5341/ingest/otlp/v1/traces"); // Seq 的 OTLP 导出路径
    ...
    .WithMetrics(metrics => metrics
        ...
        .AddOtlpExporter(options =>
        {
            options.Endpoint = new Uri("http://localhost:4318"); // Jaeger 的 OTLP HTTP 端点
        }));
...
builder.Services.AddLogging(logging => logging.AddOpenTelemetry(openTelemetryLoggerOptions =>
{
    ...
    openTelemetryLoggerOptions.AddOtlpExporter(exporter =>
    {
        ...
        exporter.Endpoint = new Uri("http://localhost:5341/ingest/otlp/v1/logs"); // 设置 OTLP 服务器端点地址，指定将日志发送到 Seq
...
var registration = new AgentServiceRegistration
{
    ID = $"{serviceName}-1",
    Name = $"{serviceName}",
    Address = hostIP, // 让 Consul 自动识别 IP
    Port = 5043,
